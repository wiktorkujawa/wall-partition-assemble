...
type materialType = 'Lumber' | 'Concrete';

export const calculateOrder = (
  wallLength: number,
  wallHeight: number = 8,
  plasterBoardType: plasterBoardType = 'Standard',
  floorMaterial: materialType = 'Lumber',
  ceilingMaterial: materialType = 'Lumber',
  connectedWallMaterial: materialType = 'Lumber',
  ...
  Object.entries(materials.fixings).map((screws)=>{
    if(screws[1]!=0)
      firstChoice.push({name: screws[0], quantity: screws[1]});
  })
...
const calculateBoards = (wallLength: number, wallHeight: number) => {
  ...
  chosenBoards = chosenBoards.map((board: any) => {
    board[1].quantity = Math.ceil(wallLength / board[1].width) + Math.ceil((wallLength - board[1].width / 2) / board[1].width);
...
const calculateStoodsAndTapingScrews = (boards: any, wallHeight: number) => {
  ...
  if(numberOfStoods==2){
    ...
    materials["fixings"].selfTapingSrews= 2*((numberOfStoods-2)*2*wallHeight + (2+numberOfStoods-1)*wallHeight);
